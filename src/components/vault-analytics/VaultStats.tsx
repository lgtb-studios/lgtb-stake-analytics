...
import StakePercentageCharts from "../charts/StakePercentageCharts";

export default function VaultStats() {
  const { tokenData, setTokenData, vaultData, setVaultData, setWalletAddress } =
    useVault();
  const [previousValue, setPreviousValue] = useState<number | null>(null);
  const { data, isLoading } = useFetchTokenMetadataAndPrice(
  ...
  const tokenRewardStats = [
    {
      ...
      value: `$${formatNumberWithCommas(
        vaultData?.current_reward_token_a_usd.toFixed(4)
      )}`,
    },
    {
      label: `${vaultData?.token_b_symbol} Rewards USD`,
      value: `$${formatNumberWithCommas(
        vaultData?.current_reward_token_b_usd.toFixed(4)
      )}`,
    },
  ];
  ...
  const vaultRewardsStats = [
    {
      ...
      value: `$${formatNumberWithCommas(
        vaultData?.current_reward_usd.toFixed(2)
      )}`,
    },
    {
      label: `Daily Rewards`,
      value: `$${formatNumberWithCommas(
        vaultData?.daily_reward_usd.toFixed(2)
      )}`,
    },
  ];
  const tokenStats = [
      ...
      value: `$${formatNumberWithCommas(
        tokenData?.token_info?.supply &&
          ...
          ? (Number(tokenData.token_info.supply) /
              Math.pow(10, tokenData.token_info.decimals)) *
              tokenData.tokenPrice
          : 0
      )}`,
    },
  ...
  const vaultTokenStats = [
    ...
    },
    {
      ...
      value: `$${formatNumberWithCommas(
        vaultData?.total_staked_amount_usd.toFixed(2)
      )}`,
    },
  ];
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                  {!tokenData?.content.links.image ? (
                    ...
                    <Image
                      ...
                      src={
                        tokenData?.content.links.image || "/default-image.png"
                      }
                      alt={
                        tokenData?.content.metadata.name || "Default Alt Text"
                      }
                      width={100}
                      height={100}
                  ...
                  <div>
                    ...
                    <IconLinks
                      poolAddress={vaultData?.pool_address}
                      tokenSymbol={vaultData?.token_a_symbol}
                    />
                  </div>
                </div>
